    ...
    try:
        ...
        for container in containers_data:
            containers.append({
                ...
                "SampleCount": container[3]
            })
        
        return render_template('sections/containers.html', containers=containers)
    except Exception as e:
...
def disposal():
    try:
        ...
        disposals_data = cursor.fetchall()
        disposals = []
        
        for disposal in disposals_data:
            disposals.append({
                "DisposalID": disposal[0],
                "SampleID": f"PRV-{disposal[1]}",
                "Description": disposal[2],
                "DisposalDate": disposal[3].strftime('%Y-%m-%d') if disposal[3] else "Ukendt",
                "AmountDisposed": disposal[4],
                "DisposedBy": disposal[5],
                "Notes": disposal[6]
            })
        
        # Hent prøver til kassation
        cursor.execute("""
            SELECT 
                s.SampleID,
                s.Description,
        ...
        print(f"Error loading disposal: {e}")
        return render_template('sections/disposal.html', error="Fejl ved indlæsning af kassationer")
